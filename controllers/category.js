...
const addCategory = async (req, res) => {
  ...
  const { categoryName, status, userId } = req.body;
  ...
  newCategory
    ...
    .then((data) => {
      ...
      res
        .status(200)
        .json({ message: "added a new category", status: true, data });
    })
    .catch((err) => {
      console.log(err);
...
const getAllCategories = async (req, res) => {
  try {
    ...
    const catList = categories.map((obj) => obj.categoryName);
    res.status(200).json({ categories: catList });
  } catch (error) {
...
const getAllCategoriesByUser = async (req, res) => {
  res.setHeader("Content-Type", "application/json");
  // console.log("hello");
  Category_Model.find({ userId: req.params.userId })
    .sort({ createdAt: -1 })
